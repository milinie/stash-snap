...
function App() {
  ...
  const handleSaveBundle = () => {
    ...
    const newBundle = {
      id: Date.now(),
      name: `Bundle ${new Date().toLocaleDateString("en-US", { month: "short", day: "numeric" })}`,
      fabrics: designWall
    };
    ...
    <div style={{ minHeight: "100vh", background: PALETTE.cream, fontFamily: "Georgia, serif", width: "100%" }}>
      ...
      <main style={{ ...contentWrap, paddingTop: 20, paddingBottom: 120 }}>
        ...
        {activeTab === "saved" && (
          <div>
            <h2>Saved Bundles 💾</h2>
            {savedBundles.length === 0 && <p style={{ color: "#999", fontFamily: "sans-serif" }}>No saved bundles yet.</p>}
            ...
            {savedBundles.map((bundle) => (
              <div key={bundle.id} style={cardStyle}>
                <div style={{ padding: 16 }}>
                  ...
                  <button onClick={() => { setDesignWall(bundle.fabrics); setActiveTab("bundles"); }} style={addWallButtonStyle}>
                    Open in Design Wall
                  </button>
...
const autoButtonStyle = { background: PALETTE.teal, color: "white", border: "none", borderRadius: 50, padding: "12px 18px", fontSize: 14, fontFamily: "sans-serif", fontWeight: 700, marginBottom: 16 };
const designWallBoxStyle = { background: "white", borderRadius: 16, padding: 16, marginBottom: 16 };
