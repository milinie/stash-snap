...
{STYLE_TAGS.map((style) => <option key={style} value={style}>{style}</option>)}
</select>
...
<textarea value={form.notes} onChange={(e) => update(“notes”, e.target.value)} rows={3} style={{ …inputStyle, resize: “none” }} />
...
// ✅ FIX: Persist stash to localStorage whenever it changes
useEffect(() => {
try {
...
// ✅ FIX: Persist savedBundles to localStorage whenever it changes
useEffect(() => {
try {
localStorage.setItem(“saved-bundles”, JSON.stringify(savedBundles));
} catch (error) {
console.error(“Could not save bundles:”, error);
}
}, [savedBundles]);
...
// ✅ FIX: handleSave only adds NEW items (no id). Edit goes through handleUpdate.
...
// ✅ FIX: handleUpdate correctly replaces the existing item by id
...
const bundleName = `Bundle ${new Date().toLocaleDateString("en-US", { month: "short", day: "numeric" })}`;
const newBundle = { id: Date.now(), name: bundleName, fabrics: designWall };
// ✅ FIX: setSavedBundles triggers the useEffect which persists automatically
...
// ✅ FIX: setSavedBundles triggers the useEffect which persists automatically
setSavedBundles((prev) => prev.filter((bundle) => bundle.id !== id));
...
<button onClick={() => { const filter = mapMissingToFilter(m); if (filter) setBundleFilter(filter); }} style={miniButtonStyle}>
Find in Stash
</button>
...
{bundle.fabrics.length} fabrics · {bundle.fabrics.reduce((sum, fabric) => sum + fabric.yardage, 0).toFixed(1)} yards
</p>
<button
...
```
  {/* ✅ FIX: Add modal uses handleSave (new items only) */}
  {adding && <AddModal onSave={handleSave} onClose={() => setAdding(false)} />}

  {/* ✅ FIX: Edit modal uses handleUpdate (updates existing by id) */}
  {editingItem && (
    <AddModal initialData={editingItem} onSave={handleUpdate} onClose={() => setEditingItem(null)} />
  )}
...
const saveModalButtonStyle = { width: “100%”, border: “none”, borderRadius: 50, padding: 16, fontSize: 17, color: “white”, background: `linear-gradient(135deg, ${PALETTE.teal}, ${PALETTE.sage})`, marginTop: 8 };
const cancelModalButtonStyle = { width: “100%”, border: “1px solid #ddd”, borderRadius: 50, padding: 14, fontSize: 15, background: “white”, color: PALETTE.ink, marginTop: 10 };
